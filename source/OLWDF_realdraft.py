...
if __name__ == '__main__':
    ...
    for DELAY_ROUNDS in delay_values:
        ...
        for current_time in range(n):
            ...
            if update_idx >= 0:
                ...
                if not np.isnan(combined_Softlabel_ensemble_soft[0]) and update_idx in buffer_feat_z:
                    _, _, current_loss_x_soft, _ = ftrl_x_ensemble_soft.fit_soft_labels(feat_indices,
                                                                                        buffer_feat_x[update_idx],
                                                                                        combined_Softlabel_ensemble_soft,
                                                                                        decay_choice,
                                                                                        contribute_error_rate)
                    _, _, current_loss_z_soft, _ = ftrl_z_ensemble_soft.fit_soft_labels(feat_indices,
                                                                                        buffer_feat_z[update_idx],
                                                                                        combined_Softlabel_ensemble_soft,
                                                                                        decay_choice,
                                                                                        contribute_error_rate)
                    ...
                    loss_x_history.append(x_soft_loss)
                    loss_z_history.append(z_soft_loss)
            ...
            if soft_labels_ensemble_all_hard is not None:
                start_idx_pseudo = current_time - len(soft_labels_ensemble_all_hard)
                ...
                for k in range(len(soft_labels_ensemble_all_hard)):
                    ...
                    if t_idx > update_idx and t_idx in buffer_feat_x and t_idx in buffer_feat_z:
                        ...
                        if not np.isnan(p_lbl_ens_soft[0]):
                            _, _, temp_step_loss_x_soft, _ = temp_ftrl_x_ensemble_soft.fit_soft_labels(feat_indices,
                                                                                                       buffer_feat_x[
                                                                                                           t_idx],
                                                                                                       p_lbl_ens_soft,
                                                                                                       decay_choice,
                                                                                                       contribute_error_rate)
                            _, _, temp_step_loss_z_soft, _ = temp_ftrl_z_ensemble_soft.fit_soft_labels(feat_indices,
                                                                                                       buffer_feat_z[
                                                                                                           t_idx],
                                                                                                       p_lbl_ens_soft,
                                                                                                       decay_choice,
            ...
            p_x_ensemble_soft, w_x_ensemble_soft = temp_ftrl_x_ensemble_soft.fit_soft_labels(feat_indices, feat_x_curr,
                                                                                             100, decay_choice,
                                                                                             contribute_error_rate)
            soft_pred_label_x = 1 if p_x_ensemble_soft > 0.5 else 0
            p_z_ensemble_soft, w_z_ensemble_soft = temp_ftrl_z_ensemble_soft.fit_soft_labels(feat_indices, feat_z_curr,
                                                                                             100, decay_choice,
                                                                                             contribute_error_rate)
            ...
            gama = 0.5
            a, b = ftrl_x_ensemble_soft.fit_soft_labels(feat_indices, feat_x_curr, 100, decay_choice,
                                                        contribute_error_rate)
            ...
            a, b = ftrl_z_ensemble_soft.fit_soft_labels(feat_indices, feat_z_curr, 100, decay_choice,
                                                        contribute_error_rate)
            ...
            results_lists['soft_X']['probs'].append(p_x_ensemble_soft)
            ...
            results_lists['soft_Z']['probs'].append(p_z_ensemble_soft)
            ...
            results_lists['resnet_soft_X']['probs'].append(t_x)
            ...
            results_lists['resnet_soft_Z']['probs'].append(t_z)

            results_lists['resnet_soft_Ens']['errors'].append(
                int(np.abs(curr_true_y - resnet_pred_label_ens_soft) > 0.5))
        ...
        evaluator = DriftEvaluator(
            ...
            dataset=dataset,
            window_size=int(DELAY_ROUNDS / 10)
        )
        ...
        evaluator.plot_curves(
            title=f"{dataset}: Impact of Label Delay ({DELAY_ROUNDS}) on Concept Drift Adaptation| BATCH_SIZE: {batch_size_denominator} | WINDOW_SIZE: {window_size_denominator}",
            BATCH_SIZE=batch_size_denominator, WINDOW_SIZE=window_size_denominator,
            save_dir="../Result/baseline_my_v1/" + dataset + "_" + str(decay_choice) + "_" + str(
                contribute_error_rate) + "_" + str(window_size_denominator) + "_" + str(batch_size_denominator))
        ...
        svm_error, _ = calculate_svm_error(X_input_zero, Y_label[:n], n)
        ...
        results['resnet_soft_X_imp_delay_missing'] = {
            'CER': resnet_soft_X_CER[-1], 'recall': resnet_soft_X_rec, 'precision': resnet_soft_X_prec,
            'f1': resnet_soft_X_f1, 'auc': resnet_soft_X_auc
        }
        results['resnet_soft_Y_imp_delay_missing'] = {
            'CER': resnet_soft_Y_CER[-1], 'recall': resnet_soft_Y_rec, 'precision': resnet_soft_Y_prec,
            'f1': resnet_soft_Y_f1, 'auc': resnet_soft_Y_auc
        }
        results['resnet_soft_Z_imp_delay_missing'] = {
            'CER': resnet_soft_Z_CER[-1], 'recall': resnet_soft_Z_rec, 'precision': resnet_soft_Z_prec,
            'f1': resnet_soft_Z_f1, 'auc': resnet_soft_Z_auc
        }
